"""Assembled Outcome v1
The assembled outcome for all components
"""
...
def add_monster_card(catalogue):
    ...
    while True:
        ...
        if ynbox(f"Is this information correct?\n"
            ...
            if not ynbox("Do you want to re-enter the details?",
                                 "Retry", ["Yes", "No"]):
                break
...
def edit_monster_card(catalogue):
    while True:
        ...
        if not monster_names:
            msgbox("No monster cards available to edit.",
                           "Empty Catalogue")
            break
        ...
        new_monster_name = enterbox("Enter new name for the "
                                            "monster card:"
                                            "", "Edit Monster Name",
                                            monster_name)
            ...
            for attribute in selected_monster:
                if attribute[0] == attribute_to_edit:
                    ...
                    if new_value is not None:
                        ...
                        msgbox(f"{attribute_to_edit} updated to "
                                       f"{new_value}"
                                       f" for {monster_name}.",
                                       "Attribute Updated")

            # Check if the user wants to continue editing
            if not ynbox("Do you want to continue editing this "
